...
class CustomRunsTestCase(TestCase):

    def run_runs_test(self, bit_sequence):
        """
        Runs Test 실행 및 자동 검증.
        - 비트 시퀀스를 입력으로 받아 NIST Runs Test 통과 여부를 검증하고 결과 출력.
        """
        # 비트 문자열을 바이트 배열로 변환
        byte_sequence = bytes([int(bit) for bit in bit_sequence])
        
        # Runs Test 실행
        result = sp80022suite.runs(byte_sequence)
        print(f"Runs Test Result: {result}")

        # 자동 검증: P-값이 0.01 이상이어야 통과
        self.assertGreaterEqual(result, 0.01, "❌ NIST Runs 테스트 실패: 랜덤성이 부족합니다.")
        
        return result

    def test_sample_file(self):
        """
        sample_bits.txt 파일에서 비트 시퀀스를 읽어와 Runs 테스트를 수행합니다.
        """
        try:
            ...
            # 테스트 실행
            self.run_runs_test(bit_sequence)
...
if __name__ == "__main__":
    import unittest
    print("🔍 Starting NIST Runs Test using sample_bits.txt...")
    # 랜덤 비트 시퀀스 생성 (필요에 따라 호출)
    CustomRunsTestCase().generate_random_bits(10000)
    unittest.main()
